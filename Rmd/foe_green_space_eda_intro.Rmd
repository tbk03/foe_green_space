...
green_space_urban_rural %>%
  ...
  scale_y_discrete(labels = c("Urban", "Town and\n Fringe", "Village and\nisolated\nhomes")) + # rename for readability
...
outlier_cutoff <- 5000
outliers_removed <- green_space_urban_rural %>% 
  filter(garden_area_per_capita >= outlier_cutoff) %>% 
  tally() %>% 
  as.integer()
...
# add plot background layer
p + add_plot_background_colours(garden_shading_boundaries) +
  add_plot_background_lines(garden_shading_boundaries) +
  
  # add the plot of the data itself
  geom_histogram(mapping = aes(x = garden_area_per_capita), fill = "grey60", colour = "grey50") +
  
  labs(title = "How much garden area per person do English Neighborhoods have?\n",
       subtitle = "",
       x = expression(paste("Garden area per capita (", m^2, " per person)")),
       y = "Number of Neighborhoods\n",
       caption = str_c("\nOutliers with garden area per capita above ", outlier_cutoff,
                       ".\n", outliers_removed, " neighborhood/s removed."),
       fill = "FoE Garden\nSpace Score")
...
p + add_plot_background_colours(garden_shading_boundaries) +
  ...
  geom_jitter(mapping = aes(x = garden_area_per_capita, 
                            y = green_space_deprivation_rating,
                            colour = as_factor(gsdi_garden)),
              alpha = 0.4) +
  
  scale_colour_brewer(palette = "YlGn") +
  
  geom_boxplot(mapping = aes(x = garden_area_per_capita, 
                             y = green_space_deprivation_rating),
               alpha = 0.1,
...
By green space deprivation rating
