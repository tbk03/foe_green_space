...
add_garden_plot_labels <- function(){
  labs(x = expression(paste("Garden area per capita (", m^2, " per person)")),
       caption = str_c(outliers_removed, " neighborhoods with garden area per capita \n between ", outlier_cutoff,
                       " and ", max_garden_space, " omitted from the plot."),
       fill = "FoE Garden\nSpace Score"
       )
}
...
outlier_cutoff <- 800
outliers_removed <- count_outliers_removed(green_space_urban_rural, garden_area_per_capita, outlier_cutoff)
max_garden_space <- round(max(green_space_urban_rural$garden_area_per_capita))
...
#      scale_colour_brewer(palette = "YlGn"), # set points to the same colour scheme as backgrounf
...
```{r}
add_public_green_space_plot_labels <- function(){
  labs(x = expression(paste("Public green space area per capita (", m^2, " per person)")),
       caption = bquote(.(outliers_removed) ~ "neighborhoods with green space area per capita between" ~
                        .(outlier_cutoff) ~"and"~ .(max_public_green_space) ~ m^2 ~ "omitted from the plot."),
       fill = "FoE Public\nGreen Space\nScore"
       )
}
...
# define cut off for outliers and count the number omitted from the plot
outlier_cutoff <- 500
outliers_removed <- count_outliers_removed(green_space_urban_rural, green_space_area_per_capita, outlier_cutoff)
max_public_green_space = round(max(green_space_urban_rural$green_space_area_per_capita))
...
p + green_space_histogram(x = green_space_area_per_capita,
  ...
  add_public_green_space_plot_labels() +
  coord_cartesian(xlim = c(0, outlier_cutoff)) +
