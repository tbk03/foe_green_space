...
msoa_and_pop_by_rating %>%
  ...
  gt(rowname_col = "Green_Space_Deprivation_Rating", groupname_col = "group") %>%
    ...
    # colour cells
    tab_style(style = list(cell_fill(color = colour_scale[[5]])),
              locations = cells_body(rows = Green_Space_Deprivation_Rating == "E")
              ) %>% 

    tab_style(style = list(cell_fill(color = colour_scale[[4]])),
              locations = cells_body(rows = Green_Space_Deprivation_Rating == "D")
              ) %>% 
  
    tab_style(style = list(cell_fill(color = colour_scale[[3]])),
              locations = cells_body(rows = Green_Space_Deprivation_Rating == "C")
              ) %>% 
  
    tab_style(style = list(cell_fill(color = colour_scale[[2]])),
              locations = cells_body(rows = Green_Space_Deprivation_Rating == "B")
              ) %>% 
    tab_style(style = list(cell_fill(color = colour_scale[[1]])),
              locations = cells_body(rows = Green_Space_Deprivation_Rating == "A")
              ) %>% 
    
    # add labels
    tab_stubhead(label = html("Green Space<br>Deprivation Rating")) %>%  
    ...
    tab_options(heading.align = "left",
                ...
                column_labels.background.color = "#EDEDED", 
                summary_row.background.color = "#EDEDED",
                row_group.background.color = "#EDEDED",
                stub.background.color = "#EDEDED") %>% 
  
    tab_style(style = list(cell_text(weight = "bold", size = 14)),
                           locations = cells_column_labels(everything())
                           ) %>% 
  
    tab_style(style = list(cell_text(weight = "bold", size = 16)),
                             locations = list(cells_stubhead(), cells_column_spanners(spanners = c("Neighbourhoods", "Population")))
                             )
